...
namespace HairProvider.Provider
{
    public  class FieldTypeProv : iFildTypeProv
    {
        ...
        {
            control = new FieldTypeControl(context);
            CheckConnect?.Invoke();
        }
        public FieldType Add(FieldType item)
        {
            FieldType temp = control.GetAll().FirstOrDefault(x=>x.Name == item.Name);
        ...
        {
            FieldType temp = control.GetAll().FirstOrDefault(x => x.Id == id);
            if (temp != null)
            {
                return temp;
            }
        ...
        {
            IList<FieldType> all = control.GetAll();
            if (all != null)
            {
                return all;
            }
